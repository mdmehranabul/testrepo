...
# %% Dataloaders
train_loaders=DataLoader(train_ds,batch_size=BATCH_SIZE)
test_loaders=DataLoader(test_ds,batch_size=BATCH_SIZE)
# %%
...
for epoch in range(NUM_EPOCH):
    train_loss=0
    for i,(supervised_data) in enumerate(train_loaders):
...
with torch.no_grad():
    for (X_test,y_test) in test_loaders:
